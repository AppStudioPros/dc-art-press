...
function Dropdown({ links, open }: { links: { label: string; href: string }[]; open: boolean }) {
  ...
  return (
    ...
    >
      <div style={{ backgroundColor: 'var(--cream)', border: '1px solid var(--border)', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
        {links.map(l => (
          <Link
            key={l.href}
            href={l.href}
            className="block px-5 py-2.5 text-sm hover:bg-[var(--card-bg)] transition-colors"
            style={{ color: 'var(--body-text)', borderBottom: '1px solid var(--border)', fontFamily: 'var(--font-garamond), Georgia, serif' }}
          >
            {l.label}
          </Link>
        ))}
...
export default function Navbar() {
  ...
  return (
    <header style={{ backgroundColor: 'var(--cream)', borderBottom: '1px solid var(--rule)', position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100 }}>
      <div className="page-wrap flex items-center justify-between h-[68px]">
        ...
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', textDecoration: 'none' }}>
          ...
          <div style={{ fontFamily: 'var(--font-rye), serif', fontSize: '1.1rem', color: 'var(--body-text)', lineHeight: 1.2 }}>
            <span style={{ display: 'block', fontSize: '0.65rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--nav-link)', fontFamily: 'var(--font-garamond), serif' }}>Dry Creek</span>
            Art Press
          </div>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          {/* Cart icon */}
          <button style={{ color: 'var(--nav-link)', background: 'none', border: 'none', cursor: 'pointer', marginLeft: '0.5rem' }} aria-label="Shopping cart">
            <ShoppingCart size={18} />
          </button>
        </nav>

        {/* Mobile toggle */}
        <button className="md:hidden" style={{ color: 'var(--nav-link)', background: 'none', border: 'none' }} onClick={() => setMobileOpen(!mobileOpen)}>
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div style={{ backgroundColor: 'var(--cream)', borderTop: '1px solid var(--border)', padding: '1rem 1.5rem' }}>
          {[
            { label: 'Home', href: '/' },
            ...
            <Link key={l.href} href={l.href} onClick={() => setMobileOpen(false)}
              className="block py-2 text-sm" style={{ color: 'var(--nav-link)', borderBottom: '1px solid var(--border)' }}>
              {l.label}
            </Link>
          ))}
        </div>
