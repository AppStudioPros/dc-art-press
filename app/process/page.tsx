  ...
  return (
    <>
      ...
      <div className="page-wrap" style={{ padding: '2rem' }}>
        <h1 style={{ fontFamily: 'var(--font-avenir), serif', color: 'var(--accent)', fontSize: '2rem', marginBottom: '0.75rem' }}>About The Process</h1>
        ...
        {sections.map(s => (
          <div key={s.title} className="cat-row">
            ...
            <div>
              <h2 style={{ fontFamily: 'var(--font-avenir), serif', color: 'var(--accent)', fontSize: '1.3rem', marginBottom: '0.75rem' }}>{s.title}</h2>
              <p style={{ fontSize: '0.9rem', marginBottom: '1rem' }}>{s.desc}</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                {s.links.map(l => (
                  <Link key={l} href={s.href || '/process'} style={{ fontSize: '0.75rem', padding: '0.3rem 0.8rem', border: '1px solid var(--border)', color: 'var(--accent)', textDecoration: 'none', backgroundColor: 'var(--white)' }}>
                    {l}
                  </Link>
                ))}
              </div>
